...
from registration.backends.default.views import (RegistrationView as OldRegistrationView,
                                                 ...
                                                 ResendActivationView as OldResendActivationView)
...
class RegistrationView(OldRegistrationView):
    ...
    def get_initial(self, *args, **kwargs):
        ...
        initial['timezone'] = getattr(settings, 'DEFAULT_USER_TIME_ZONE', 'Asia/Shanghai')
        initial['language'] = Language.objects.get(key=getattr(settings, 'DEFAULT_USER_LANGUAGE', 'CPP11'))
        return initial
...
class ResendActivationView(OldResendActivationView):
    title = _('Registration')
    ...
    def get_context_data(self, **kwargs):
        if 'title' not in kwargs:
            kwargs['title'] = self.title
        return super(ResendActivationView, self).get_context_data(**kwargs)
